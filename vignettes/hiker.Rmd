...
To convert a `terrain` to a `SpatRaster`, use `hf_rasterize()`. This is mostly useful for plotting.

```{r, fig.width = 800/72}

par(mfrow = c(1, 2))

plot(red_butte_dem, main = "Elevation (meters)")
plot(hf_rasterize(terrain), main = "Travel cost (seconds)")
...
plot(hf_rasterize(terrain),  main = "No barrier")
plot(hf_rasterize(terrain2), main = "Barrier")
...
plot(hf_rasterize(terrain),  main = "No channel")
plot(hf_rasterize(terrain3), main = "Channel")
...
plot(crop(accumulated_cost, bb8))
plot(st_geometry(from), pch = 19, col = "red2", add = TRUE)
